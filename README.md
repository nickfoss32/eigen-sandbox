...
- An IMM (Interacting Multiple Model) orbital demo that fuses space-based optical measurements (RA/Dec) with multiple EKFs.
...
* `imm-demo` - Runs a 3-model IMM (`TwoBodyGravity`, `J2Gravity`, `J2PlusDrag`) against synthetic space-based optical measurements and writes JSON output.
...
- `summary.imm`: Model metadata, transition matrix, initial/final probabilities.
