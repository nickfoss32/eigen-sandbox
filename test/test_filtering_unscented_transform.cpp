...
TEST(UnscentedTransformTest, WeightsAreNormalized) {
    const auto weights = filtering::UnscentedTransform::compute_weights(6);
...
TEST(UnscentedTransformTest, SigmaPointsContainMeanAndCorrectCount) {
    ...
    const auto sigma_points =
        filtering::UnscentedTransform::generate_sigma_points(mean, covariance);

    ASSERT_EQ(sigma_points.size(), 7);
...
TEST(UnscentedTransformTest, RecoverGaussianPreservesOriginalMoments) {
    ...
    const auto weights = filtering::UnscentedTransform::compute_weights(3);
    const auto sigma_points =
        filtering::UnscentedTransform::generate_sigma_points(mean, covariance);
    const auto moments =
...
TEST(UnscentedTransformTest, TransformDistributionRunsFullRoundInOneCall) {
    ...
    EXPECT_EQ(result.source_sigma_points.size(), 7);
    EXPECT_EQ(result.transformed_sigma_points.size(), 7);
    EXPECT_EQ(result.weights.mean.size(), 7);
    EXPECT_EQ(result.weights.covariance.size(), 7);
...
TEST(UnscentedTransformTest, CrossCovarianceMatchesLinearMapping) {
    ...
    const auto weights = filtering::UnscentedTransform::compute_weights(3);
    const auto sigma_x =
        filtering::UnscentedTransform::generate_sigma_points(mean_x, cov_x);
